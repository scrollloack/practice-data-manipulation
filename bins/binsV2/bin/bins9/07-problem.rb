...
def find_latest_feedbacks_with_chain(staffs)
  staffs.map do |staff|
    ...
    staff.slice(:email, :name).merge(latest_comment: latest&.dig(:comment) || 'No available feedback yet.')
  end
end
...
def find_latest_feedbacks_with_loop(staffs)
  ...
  staffs.each do |staff|
    ...
    latest_feedbacks << staff.slice(:email, :name).merge(latest_comment: latest&.dig(:comment) || 'No available feedback yet.')
  end
