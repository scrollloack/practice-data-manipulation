...
const getUsersPostsWithObj = (): UserPosts[] => {
    const groupedPosts: Partial<Record<User['id'], Post[]>> = Object.groupBy(
        posts,
        (post) => post.user_id,
    );
...
const getUsersPostsWithLoop = (): UserPosts[] => {
    ...
    for (const { user_id, id, title } of posts) {
        const groupedPosts: PostItem[] = postsByUserId.get(user_id) ?? [];

        if (!postsByUserId.has(user_id)) {
            postsByUserId.set(user_id, groupedPosts);
        }

        groupedPosts.push({ id, title });
    }
