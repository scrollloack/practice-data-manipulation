...
type Post = Entity & { user_id: User['id']; title: string };

type PostData = { users: User[]; posts: Post[] };
...
const { users, posts }: PostData = data;

const getUsersPostsWithObj = (): UserPosts[] => {
  const groupedPosts: Partial<Record<string, Post[]>> = Object.groupBy(
    posts,
    (post) => post.user_id,
  );
...
const getUsersPostsWithLoop = (): UserPosts[] => {
  ...
  for (const { user_id, ...post } of posts) {
    const group: PostItem[] = postsByUserId.get(user_id) || [];
